...
use crate::draw;
use crate::collision::{self, Vec2d, ScreenLine, ScreenPoint};
use crate::collision::collision_object::{ CollisionLine, Boundaries, Ball, Balls}; 
...
pub fn run_demo(){
    ...
    let mut balls = Balls{
        balls: vec![
            Ball{
                position: ScreenPoint {x:50, y:100},
                radius: 15.0,
                velocity: Vec2d{x:1.0,y:0.0},
            },
            Ball{
                position: ScreenPoint {x:80, y:80},
                radius: 15.0,
                velocity: Vec2d{x:2.0,y:0.0},
            },
            Ball{
                position: ScreenPoint {x:140, y:140},
                radius: 15.0,
                velocity: Vec2d{x:3.0,y:0.0},
            },
        ]
    };

    let left_top_point = ScreenPoint{x:0, y:0};
    let right_top_point = ScreenPoint{x:800, y:0};
    let left_bottom_point = ScreenPoint{x:0, y:600};
    let right_bottom_point = ScreenPoint{x:800, y:600};
    ...
    let right_wall = ScreenLine{
    point1: right_top_point,
    point2: right_bottom_point};
    let bottom_wall = ScreenLine{
        ...
        match event {
            ...
            Event::RedrawRequested(_) => {
                ...
                for ball in &balls.balls{
                    ...
                    circles_to_draw.push((center, radius, [0, 255, 255, 255]));
                }
                
                for line in lines_to_draw {
                    draw::draw_line(frame, SCREEN_WIDTH as usize, SCREEN_HEIGHT as usize, &line);
                }
                for (center, radius, color) in circles_to_draw {
                    draw::draw_circle(frame, SCREEN_WIDTH as usize, SCREEN_HEIGHT as usize, &center, radius as i32, color);
                }
